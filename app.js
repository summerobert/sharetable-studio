...
function renderMedia(post, size) {
  ...
  if (!mediaItems.length) {
    const placeholderText = post.mediaType?.startsWith("video/") && post.mediaName
      ? `Video: ${post.mediaName}`
      : post.mediaName || post.platform;
    return `<div class="media-placeholder ${size}"><span>${escapeHtml(placeholderText)}</span></div>`;
  }
...
function openShareDialog() {
  ...
  document.getElementById("share-code").value = createShareCode();
...
function importShareCode() {
  ...
  try {
    ...
    render();
    document.getElementById("share-code").value = createShareCode();
    status.textContent = "Plán je načítaný. Fotky zo share kódu sa zobrazia v náhľadoch.";
  } catch {
...
function createShareCode() {
  const payload = {
    ...
    posts: posts.map(serializablePost),
  };
