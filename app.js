  ...
  panel.innerHTML = `
    <article class="large-post ${info.className}">
      ...
      <div class="duplicate-box">
        <strong>Publikovať aj inde</strong>
        <div class="duplicate-grid">
          <label>
            Sieť
            <select id="duplicate-platform">
              ${Object.keys(platformInfo)
                .map(
                  (platform) =>
                    `<option ${platform === getNextPlatform(selected.platform) ? "selected" : ""}>${escapeHtml(platform)}</option>`,
                )
                .join("")}
            </select>
          </label>
          <label>
            Dátum
            <input id="duplicate-date" type="date" value="${escapeHtml(selected.date)}" />
          </label>
          <label>
            Čas
            <input id="duplicate-time" type="time" value="${escapeHtml(getSuggestedDuplicateTime(selected.time || "09:00"))}" />
          </label>
        </div>
        <button type="button" class="secondary" onclick="duplicatePost('${selected.id}')">Pridať ako ďalší post</button>
      </div>
...
window.duplicatePost = (id) => {
  const post = posts.find((item) => item.id === id);
  if (!post) return;
  const platform = document.getElementById("duplicate-platform")?.value || getNextPlatform(post.platform);
  const date = document.getElementById("duplicate-date")?.value || post.date;
  const time = document.getElementById("duplicate-time")?.value || getSuggestedDuplicateTime(post.time || "09:00");
  const duplicate = {
    ...post,
  ...
  savePosts();
  render();
  setFormStatus(`Post je pridaný aj na ${platform} o ${time}.`);
};
...
function getSuggestedDuplicateTime(time) {
  const [hours = "9", minutes = "00"] = String(time || "09:00").split(":");
  const nextHours = (Number(hours) + 3) % 24;
  return `${String(nextHours).padStart(2, "0")}:${String(minutes || "00").padStart(2, "0")}`;
}
